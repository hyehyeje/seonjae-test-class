    ...
    <section className="pt-10 pb-8">
      ...
      <div className="text-center relative">
        ...
        <span className="flex justify-center items-end font-cafe24surround text-3xl font-bold p-1 text-stroke-1 text-stroke-black text-white">
          <span className="block text-5xl text-mbti-pink transform rotate-12 origin-right">
            궁
          </span>
          합{" "}
          <span className="block text-5xl text-mbti-light-pink transform -rotate-12 origin-left">
            테
          </span>
          <span className="block transform -rotate-3 origin-bottom-left">
            스트
          </span>
        </span>
      </div>
      ...
      <div className="py-8 px-5">
        <div className="flex justify-end items-center gap-1 pb-1">
          <img
            src="./images/icon/main-icon-youtube.svg"
            alt="youtube-icon"
            className="w-8"
          />
          <a
            href="https://www.youtube.com/@chutzrit"
            className="font-cafe24surround mbti-red text-mbti-red underline"
          >
            By. 후츠릿
          </a>
        </div>
        <div className="relative z-10">
          ...
          <div className="flex bg-mbti-purple border-2 border-black px-4 py-3 gap-3">
            {appleButtonList.map((button, index) => (
              <span
                key={index}
                className={`block text-no w-5 h-5 ${button.color} ${
                  button.borderSize === 1
                    ? "border"
                    : `border-${button.borderSize}`
          ...
          {/* 이미지 */}
          <div
            className="w-full relative bg-mbti-deep-yellow overflow-hidden border-2 border-black border-t-0"
            style={{ paddingBottom: "calc(100% - 32px)" }}
          >
            <img
              src="./images/hero.jpeg"
              className="absolute w-full transform scale-110 -mt-2"
              alt="'선재 업고 튀어' / tvN 제공"
            />
          </div>
        </div>
